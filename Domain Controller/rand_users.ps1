...
# Default UserCount set to 15 (if not set)
if ( $UserCount -eq 0 ) {
  $UserCount = 15
}
...
if ( $LocalAdminCount -ne 0) {
  ...
  while (($local_admin_indexes | Measure-Object ).Count -lt $LocalAdminCount) {
    ...
    Write-Host -ForegroundColor Yellow "[*] adding $random_index to local_admin_indexes $local_admin_indexes"
  }
}
...
for ( $i = 1; $i -le $UserCount; $i++ ) {
  ...
  if ( $local_admin_indexes | Where-Object { $_ -eq $i } ) {
    Write-Host -ForegroundColor Green "[+] user $i is local admin"
...
ConvertTo-Json -InputObject @{ 
  ...
  Write-Host -ForegroundColor Green "[+] Created Schema in Current Directory"